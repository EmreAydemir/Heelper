...
public class WordToSpan {
    ...
    public WordToSpan setMyLink(String txt, String link) {
        this.ws = new SpannableString(txt);


        if (txt.contains(link)) {
            int st = txt.indexOf(link);
            int en = st + link.length();
            this.ws.setSpan(new myClickableSpan(colorCUSTOM, underlineCUSTOM, "link"), st, en, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
